...
void Render3D(/*Bool useaa*/)
{
	...
	for (y = HEIGHT-1; y >= 0; y--)
	{
		...
		for (x = 0; x < WIDTH; x++)
		{
			...
			lev = ((red >= 192) || (blue >= 192) || (green >= 192)) ? 1 : 0;
			red = (red >= 80) ? 1 : 0;
			blue = (blue >= 80) ? 1 : 0;
			green = (green >= 80) ? 1 : 0;
			col = COLOR4(lev, red, green, blue);

			// set pixel
			if ((x & 1) == 0)
			{
				// set even pixel
				SETPIXEL4_EVEN(dst, col);
			}
			else
			{
				// set odd pixel
				SETPIXEL4_ODD(dst, col);
				dst++;
			}
		}
