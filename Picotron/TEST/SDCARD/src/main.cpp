	...
	while (True)
	{
		...
		if (SDType != SD_NONE)
		{
			...
			if (DiskMounted())
			{
				...
				y += 16;
#else
				DrawText("Press Spc to list files in ROOT", 0, y, COL_GREEN);
				y += 16;
				DrawText("Press Z to HEX view file LOADER.UF2", 0, y, COL_GREEN);
				y += 16;
				DrawText("Press Ctrl to write file $TEMP$.TXT", 0, y, COL_GREEN);
				y += 16;
#endif
			}
		}
