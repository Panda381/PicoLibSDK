...
#define RINGTX(name,port,spinlock,order,dma,dreq,entry) __attribute__((aligned(1<<(order)))) u8 name ## _buf[1<<(order)]; \
	sRingTx name = { name ## _buf, 1<<(order), 0, (port), (spinlock), (order), (dma), (dreq), (entry) }
