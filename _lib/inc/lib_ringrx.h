...
#define RINGRX(name,port,spinlock,order,dma,dreq,entry) __attribute__((aligned(1<<(order)))) u8 name ## _buf[1<<(order)]; \
	sRingRx name = { name ## _buf, 1<<(order), 0, (port), (spinlock), (order), (dma), (dreq), (entry) }
...
//   dreq = DMA channel data request DREQ_*
//   entry = entry size DMA_SIZE_*
void RingRxInit(sRingRx* ring, u8* buf, volatile u32* port, int spinlock, u8 order, u8 dma, u8 dreq, u8 entry);
