...
{
	if ((w <= 0) || (h <= 0)) return;
	CanvasRect(canvas, x, y, w-1, 1, col);
	CanvasRect(canvas, x+w-1, y, 1, h-1, col);
	CanvasRect(canvas, x+1, y+h-1, w-1, 1, col);
	CanvasRect(canvas, x, y+1, 1, h-1, col);
}

// clear canvas (fill with black color)
void CanvasClear(sCanvas* canvas)
{
	CanvasRect(canvas, 0, 0, canvas->w, canvas->h, 0);
}
...
//       B4 .  |  . B7
//         . B5|B6 .
void CanvasFillCircle(sCanvas* canvas, int x0, int y0, int r, u8 col, u8 mask /*=0xff*/)
{
	int x, y;
	if (r <= 0) return;
	int r2 = r*(r-1);
	...
	if (mask == 0xff)
	{
		for (y = -r; y <= r; y++)
		{
			for (x = -r; x <= r; x++)
			{
				if ((x*x + y*y) <= r2) CanvasPoint(canvas, x+x0, y+y0, col);
			}
		}
		return;
	}
	...
	if ((mask & B0) != 0)
	{
		for (y = -r; y <= 0; y++)
		{
			for (x = 0; x <= r; x++)
			{
				if (((x*x + y*y) <= r2) && (x >= -y)) CanvasPoint(canvas, x+x0, y+y0, col);
			}
		}
	}
...
//       B4 .  |  . B7
//         . B5|B6 .
void CanvasCircle(sCanvas* canvas, int x0, int y0, int r, u8 col, u8 mask /*=0xff*/)
{
	int x, y;
	if (r <= 0) return;
	r--;
	...
	while (x <= y)
	{
		if ((mask & B0) != 0) CanvasPoint(canvas, x0+y, y0-x, col); // octant 0
		if ((mask & B1) != 0) CanvasPoint(canvas, x0+x, y0-y, col); // octant 1
		if ((mask & B2) != 0) CanvasPoint(canvas, x0-x, y0-y, col); // octant 2
		if ((mask & B3) != 0) CanvasPoint(canvas, x0-y, y0-x, col); // octant 3
		if ((mask & B4) != 0) CanvasPoint(canvas, x0-y, y0+x, col); // octant 4
		if ((mask & B5) != 0) CanvasPoint(canvas, x0-x, y0+y, col); // octant 5
		if ((mask & B6) != 0) CanvasPoint(canvas, x0+x, y0+y, col); // octant 6
		if ((mask & B7) != 0) CanvasPoint(canvas, x0+y, y0+x, col); // octant 7
...
void CanvasSmallText(sCanvas* canvas, const char* text, int x, int y, u8 col,
	const void* font, int fontheight /*=8*/, int fontwidth /*=6*/)
{
	u8 ch;
	int i, j;
	const u8* s;
	...
	while ((ch = (u8)*text++) != 0) // until end of text
	{
		...
		for (i = fontheight; i > 0; i--)
		{
			...
			// loop through pixels of one character line
			for (j = fontwidth; j > 0; j--)
			{
				// pixel is set
				if ((ch & 0x80) != 0) CanvasPoint(canvas, x, y, col);
				ch <<= 1;
				x++;
			}
			x -= fontwidth;
...
void CanvasBlit(sCanvas* canvas, sCanvas* src, int xd, int yd, int xs, int ys, int w, int h, u8 col)
{
	...
	{
		CanvasImg(canvas, src, xd, yd, xs, ys, w, h);
		return;
	}
...
//  color ... key or border color (CANVASIMG_PERSP mode: horizon offset)
	...
	const sMat2D* m, u8 mode, u8 color)
{
	...
	// limit y
	int h0 = h;
	int y0 = (mode == CANVASIMG_PERSP) ? (-h) : (-h/2); // start Y coordinate
	if (y < 0)
	{
		h += y;
		y0 -= y;
	...
	// wrap image
	if (mode == CANVASIMG_WRAP)
	{

#if DRAW_HWINTER // 1=use hardware interpolator to draw images

		// configure lane 0 to add accumulator 0 to base 0 .... X coordinate
 		InterpReset(DRAW_HWINTER_INX);	// reset interpolator
		InterpAddRaw(DRAW_HWINTER_INX, 0, True); // bypass mask+shift of lane 0
		InterpShift(DRAW_HWINTER_INX, 0, FRACT); // shift fraction bits out
		int xbits = 0;
		while ((1<<xbits)<src->w) xbits++; // get number of bits of image width
		InterpMask(DRAW_HWINTER_INX, 0, 0, xbits-1); // set x mask (0..width)

		// configure lane 1 to mask to Y coordinate
		int ybits = 0;
		while ((1<<ybits)<src->h) ybits++; // get number of bits of image height
		InterpShift(DRAW_HWINTER_INX, 1, FRACT - xbits); // shift fraction bits out, to multiply * width
		InterpMask(DRAW_HWINTER_INX, 1, xbits, xbits+ybits-1); // set y mask, multiply * width

		// set image base address
		InterpBase(DRAW_HWINTER_INX, 2, (u32)s);
		...
		for (; h > 0; h--)
		{
			...
			InterpAccum(DRAW_HWINTER_INX, 0, xy0m);
			InterpBase(DRAW_HWINTER_INX, 0, m11);
			InterpAccum(DRAW_HWINTER_INX, 1, yy0m);
			InterpBase(DRAW_HWINTER_INX, 1, m21);

			for (i = w; i > 0; i--)
			{
				*d++ = *(u8*)InterpPop(DRAW_HWINTER_INX, 2);
			}
	...
	// no border
	else if (mode == CANVASIMG_NOBORDER)
	{
	...
	else if (mode == CANVASIMG_PERSP)
	{
		...
		for (; h > 0; h--)
		{
			...
			InterpAccum(DRAW_HWINTER_INX, 0, xy0m);
			InterpBase(DRAW_HWINTER_INX, 0, m11b);
			InterpAccum(DRAW_HWINTER_INX, 1, yy0m);
			InterpBase(DRAW_HWINTER_INX, 1, m21b);
	...
	int tilebits, int x, int y, int w, int h, const sMat2D* mat, u8 horizon)
{
	...
	InterpReset(0);	// reset interpolator 0
	InterpAddRaw(0, 0, True); // bypass mask+shift of lane 0
	InterpShift(0, 0, FRACT + tilebits); // shift to get tile index X
	InterpMask(0, 0, 0, mapwbits-1);
	InterpShift(0, 1, FRACT + tilebits - mapwbits); // shift to get tile index Y
	InterpMask(0, 1, mapwbits, mapwbits+maphbits-1);
	InterpBase(0, 2, (u32)map); // map base

	// prepare hardware interpolator 1 to get pixel index
	InterpReset(1);	// reset interpolator 1
	InterpShift(1, 0, FRACT); // shift to get pixel index X
	InterpMask(1, 0, 0, tilebits-1);
	InterpShift(1, 1, FRACT - tilebits); // shift to get pixel index Y
	InterpMask(1, 1, tilebits, tilebits2-1);
	InterpBase(1, 2, (u32)s); // tile image
	...
	for (; h > 0; h--)
	{
		...
		InterpAccum(0, 0, xy0m);
		InterpBase(1, 0, xy0m);
		InterpAccum(0, 1, yy0m);
		InterpBase(1, 1, yy0m);


		for (i = w; i > 0; i--)
		{
			u8* map = (u8*)InterpPop(0, 2);
			u8* base = (u8*)InterpPop(1, 2);
...
void CanvasImgLine(sCanvas* canvas, sCanvas* src, int xd, int yd, int xs, int ys, int wd, int ws)
{
	...
	InterpAddRaw(DRAW_HWINTER_INX, 0, True); // bypass mask+shift of lane 0
	InterpShift(DRAW_HWINTER_INX, 0, FRACT); // shift fraction bits out
	InterpBase(DRAW_HWINTER_INX, 0, dinc); // pixel increment
	InterpAccum(DRAW_HWINTER_INX, 0, 0); // base source
	InterpBase(DRAW_HWINTER_INX, 2, (u32)s); // image base
