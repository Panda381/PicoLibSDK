 ...
 *
 * Inputs:      filled sMP3DecInfo structure, after calling IMDCT for all channels
...
int Subband(sMP3DecInfo *mp3DecInfo, short *pcmBuf)
{
	...
	sbi = (sMP3SubbandInfo*)(mp3DecInfo->SubbandInfoPS);
	...
	if (mp3DecInfo->nChans == 2) {
		...
		for (b = 0; b < MP3_BLOCK_SIZE; b++) {
			...
			PolyphaseStereo(pcmBuf, sbi->vbuf + sbi->vindex + MP3_VBUF_LENGTH * (b & 0x01), polyCoef);
		...
		for (b = 0; b < MP3_BLOCK_SIZE; b++) {
			FDCT32(mi->outBuf[0][b], sbi->vbuf + 0*32, sbi->vindex, (b & 0x01), mi->gb[0]);
			PolyphaseMono(pcmBuf, sbi->vbuf + sbi->vindex + MP3_VBUF_LENGTH * (b & 0x01), polyCoef);
			sbi->vindex = (sbi->vindex - (b & 0x01)) & 7;
			pcmBuf += MP3_NBANDS;
		}
	}
