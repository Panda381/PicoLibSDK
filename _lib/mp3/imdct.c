 ...
 *
 * Inputs:      output vector y (18 new samples, spaced MP3_NBANDS apart)
...
static int FreqInvertRescale(int *y, int *xPrev, int blockIdx, int es)
{
	...
	if (es == 0) {
		...
		if (blockIdx & 0x01) {
			/* frequency invert */
			for (i = 0; i < 18; i+=2) {
				d = *y;		CLIP_2N(d, 31 - es);	*y = d << es;	mOut |= FASTABS(*y);	y += MP3_NBANDS;
				d = -*y;	CLIP_2N(d, 31 - es);	*y = d << es;	mOut |= FASTABS(*y);	y += MP3_NBANDS;
 ...
 *              sMP3SideInfoSub struct for this granule/channel
 ...
 **************************************************************************************/
static int HybridTransform(int *xCurr, int *xPrev, int y[MP3_BLOCK_SIZE][MP3_NBANDS], sMP3SideInfoSub *sis, BlockCount *bc)
{
	int xPrevWin[18], currWinIdx, prevWinIdx;
	int i, j, nBlocksOut, nonZero, mOut;
	int fiBit, xp;

	ASSERT(bc->nBlocksLong  <= MP3_NBANDS);
	ASSERT(bc->nBlocksTotal <= MP3_NBANDS);
	ASSERT(bc->nBlocksPrev  <= MP3_NBANDS);
 ...
 *
 * Inputs:      sMP3DecInfo structure filled by UnpackFrameHeader(), UnpackSideInfo(),
 ...
 **************************************************************************************/
 // a bit faster in RAM
/*__attribute__ ((section (".data")))*/ int IMDCT(sMP3DecInfo *mp3DecInfo, int gr, int ch)
{
	...
	hi = (sMP3HuffmanInfo*)(mp3DecInfo->HuffmanInfoPS);
	mi = (sMP3IMDCTInfo *)(mp3DecInfo->IMDCTInfoPS);
	 ...
	 */
	blockCutoff = fh->sfBand->l[(fh->ver == MP3_VER_MPEG1 ? 8 : 6)] / 18;	/* same as 3* num short sfb's in spec */
	...
	ASSERT(mi->numPrevIMDCT[ch] <= MP3_NBANDS);
