 ...
 *
 * Inputs:      sample buffer (decoded Huffman codewords), length = MP3_MAX_NSAMP samples
 *              work buffer for reordering short-block, length = MP3_MAX_REORDER_SAMPS
 *                samples (3 * width of largest short-block critical band)
 *              non-zero bound for this channel/granule
 *              valid sMP3FrameHeader, sMP3SideInfoSub, sMP3ScaleFactorInfoSub, and sMP3CriticalBandInfo
 *                structures for this channel/granule
 *
 * Outputs:     MP3_MAX_NSAMP dequantized samples in sampleBuf
 ...
 **************************************************************************************/
/*__attribute__ ((section (".data")))*/ int DequantChannel(int *sampleBuf, int *workBuf, int *nonZeroBound, sMP3FrameHeader *fh, sMP3SideInfoSub *sis, 
					sMP3ScaleFactorInfoSub *sfis, sMP3CriticalBandInfo *cbi)
{
	...
	if (sis->blockType == 2) {
		if (sis->mixedBlock) { 
			cbEndL = (fh->ver == MP3_VER_MPEG1 ? 8 : 6); 
			cbStartS = 3; 
		} else {
			cbEndL = 0; 
			cbStartS = 0;
	...
	for (cb = cbStartS; cb < cbEndS; cb++) {
		...
		ASSERT(3*nSamps <= MP3_MAX_REORDER_SAMPS);
	...
	*nonZeroBound = i;
	...
	ASSERT(*nonZeroBound <= MP3_MAX_NSAMP);
