...
/*__attribute__ ((section (".data")))*/ void FDCT32(int *buf, int *dest, int offset, int oddBlock, int gb)
{
	...
	d = dest + 64*16 + ((offset - oddBlock) & 7) + (oddBlock ? 0 : MP3_VBUF_LENGTH);
	s = buf[ 0];				d[0] = d[8] = s;
    
	/* samples 16 to 31 */
	d = dest + offset + (oddBlock ? MP3_VBUF_LENGTH  : 0);
	...
	d = dest + 16 + ((offset - oddBlock) & 7) + (oddBlock ? 0 : MP3_VBUF_LENGTH);
