...
// abort DMA transfer
void NOFLASH(DMA_Abort_RAM)(u8 chan)
{
...
void NOFLASH(PioFifoClear_RAM)(u8 pio, u8 sm)
{
...
void NOFLASH(PioSetAddr_RAM)(u8 pio, u8 sm, u8 addr)
{
...
int NOFLASH(VgaBufProcess)()
{
	...
	if (y0 >= 0)
	{
		...
		for (layer = 1; layer < LAYERS; layer++)
		{
			...
			// stop DMA channel
			DMA_Abort_RAM(VGA_DMA_PIO(layer));
			DMA_Abort_RAM(VGA_DMA_CB(layer));
			DMA_Abort_RAM(VGA_DMA_PIO(layer));
			DMA_Abort_RAM(VGA_DMA_CB(layer));

			// restart state machine and clear FIFOs
			PioSMDisable(VGA_PIO, sm);
			PioFifoClear_RAM(VGA_PIO, sm);
			PioSMRestart(VGA_PIO, sm);
//			PioSetAddr_RAM(VGA_PIO, sm, CurLayerProg.idle+LAYER_OFFSET);
			PioSetAddr_RAM(VGA_PIO, sm, CurLayerProg.entry+LAYER_OFFSET);
...
void VgaPioInit()
{
	...
	PioSetupGPIO(VGA_PIO, VGA_GPIO_HSYNC, 1);

	// negative HSYNC output
	if (!CurVmode.psync) GPIO_OutOverInvert(VGA_GPIO_HSYNC);
	...
	for (layer = 0; layer < LAYERS; layer++)
	{
		...
		PioSetPinDir(VGA_PIO, VGA_SM(layer), VGA_GPIO_HSYNC, 1, 1);
		...
		if (layer == 0)
		{
			...
			PioSetupSideset(VGA_PIO, VGA_SM(layer), VGA_GPIO_HSYNC, 1, False, False);
...
void VgaBufInit()
{
	...
	else
	{
		...
		{
			// vertical synchronization as part of CSYNC
			//   hsync must be min. 4
