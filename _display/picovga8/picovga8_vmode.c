...
void VgaCfgDef(sVgaCfg* cfg)
{
	...
	cfg->vsync = False;		// use separate VSYNC instead of CSYNC (only progressive mode)
}
...
void VgaCfg(const sVgaCfg* cfg, sVmode* vmode)
{
	...
	// use separate VSYNC instead of CSYNC (only progressive mode)
	vmode->vsync = cfg->vsync;
	...
	PllCalcDef(freq, &freq, &vco, &fbdiv, &pd1, &pd2);
...
// timings
const sVideo* VideoResTab[3*RES_MAX] =
{
	// DEV_VGA, DEV_OLDVGA
...
void Video(u8 dev, u8 res, u8 form, u8* buf, const void* buf2 /* = FontBoldB8x16 */)
{
	...
	// auto device
//	if (dev >= DEV_AUTO) dev = GetDispCfg();
	...
	u32 freq = 120000; // required minimal system frequency in kHz (real frequency can be higher)
	if ((form == FORM_TEXT8) || (form == FORM_MTEXT8))
	{
		w = w/8*8;
		h = h/8*8;
		if (w > 400) freq = 150000;
	}
	...
	Cfg.vsync = vsync; // use separate VSYNC
	...
	ClockPllSysSetup(Vmode.fbdiv, Vmode.pd1, Vmode.pd2);
