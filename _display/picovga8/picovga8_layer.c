...
//		.prg=&vga_program,			// pointer to program descriptor
		.length=count_of(vga_program_instructions), //vga_program.length,		// program length (number of instructions)
...
//		.prg=&keylayer_program,			// pointer to program descriptor
		.length=count_of(keylayer_program_instructions), //keylayer_program.length,	// program length (number of instructions)
...
//		.prg=&blacklayer_program,		// pointer to program descriptor
		.length=count_of(blacklayer_program_instructions), //blacklayer_program.length,	// program length (number of instructions)
...
//		.prg=&whitelayer_program,		// pointer to program descriptor
		.length=count_of(whitelayer_program_instructions), //whitelayer_program.length,	// program length (number of instructions)
...
//		.prg=&monolayer_program,		// pointer to program descriptor
		.length=count_of(monolayer_program_instructions), //monolayer_program.length,	// program length (number of instructions)
...
//		.prg=&rlelayer_program,			// pointer to program descriptor
		.length=count_of(rlelayer_program_instructions), //rlelayer_program.length,	// program length (number of instructions)
...
// number of pins of overlapped layer (base layer should stay VGA_GPIO_NUM)
u8 LayerNumPin[LAYERS_MAX] = { VGA_GPIO_NUM, VGA_GPIO_NUM, VGA_GPIO_NUM, VGA_GPIO_NUM};
...
void LayerSetup(u8 inx, const u8* img, const sVmode* vmode, u16 w, u16 h, u8 col, const void* par)
{
...
{
	LayerSetup(inx, (const u8*)sprite, vmode, w, h, col, NULL);
	LayerSetX(inx, x);
	LayerSetY(inx, y);
	sLayer* lay = &LayerScreen[inx]; // get pointer to layer
