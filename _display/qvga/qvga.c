...
#define QVGACMD(jmp, num) ( ((u32)((jmp)+QVGA_PIO_OFF)<<27) | (u32)(num))
...
void QVgaPioInit()
{
	...
	// load PIO program
	PioLoadProg(QVGA_PIO, qvga_program_instructions, count_of(qvga_program_instructions), QVGA_PIO_OFF);

	// PIO set wrap address
	PioSetWrap(QVGA_PIO, QVGA_SM, QVGA_PIO_OFF + qvga_wrap_target, QVGA_PIO_OFF + qvga_wrap);

	// set start address
	PioSetAddr(QVGA_PIO, QVGA_SM, QVGA_PIO_OFF + qvga_offset_entry);
...
void DispUpdate()
{
	...
	{
		// copy data
		u8* d = &FrameBuf[DispDirtyX1 + DispDirtyY1*WIDTH];
		u8* s = &BackBuf[DispDirtyX1 + DispDirtyY1*WIDTH];
		int i;
