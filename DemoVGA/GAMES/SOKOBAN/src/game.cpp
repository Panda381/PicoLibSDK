...
Bool Step(char key)
{
	int dx = (key == CH_LEFT) ? -1 : ((key == CH_RIGHT) ? 1 : 0);
	int dy = (key == CH_UP) ? -1 : ((key == CH_DOWN) ? 1 : 0);
	...
	int dir = (key == CH_LEFT) ? DIR_L :
		((key == CH_RIGHT) ? DIR_R :
		((key == CH_UP) ? DIR_U : DIR_D));
...
void GameLoop()
{
	...
	while (True)
	{
		...
		if (!FastMode)
		{
			...
			if (i == NOKEY)
			{
				if (UsbKeyIsPressed(HID_KEY_ARROW_LEFT)) i = CH_LEFT;
				if (UsbKeyIsPressed(HID_KEY_ARROW_RIGHT)) i = CH_RIGHT;
				if (UsbKeyIsPressed(HID_KEY_ARROW_UP)) i = CH_UP;
				if (UsbKeyIsPressed(HID_KEY_ARROW_DOWN)) i = CH_DOWN;
			}
