...
MOVE_AGAIN:
	...
	while (True)
	{
		...
		if (PlayerView)
		{
			if ((ch == CH_LEFT) || (ch == CH_RIGHT)) ch ^= CH_LEFT ^ CH_RIGHT;
			if ((ch == CH_UP) || (ch == CH_DOWN)) ch ^= CH_UP ^ CH_DOWN;
		}
...
#define MENUX 30
	...
	DrawText("SPACE ... Select level:", MENUX, 195, COL_WHITE);
