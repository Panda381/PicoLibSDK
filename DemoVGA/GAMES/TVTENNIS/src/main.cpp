...
void GameControl()
{
	...
	{
		autoy = BallY + CompY[0];
		if (PaddleY[0] < autoy) key0 = HID_KEY_ARROW_DOWN;
		if (PaddleY[0] > autoy) key0 = HID_KEY_ARROW_UP;
	}

	if ((Comp[1]) && (BallX >= CompX[1]))
	{
		autoy = BallY + CompY[1];
		if (PaddleY[1] < autoy) key1 = HID_KEY_ARROW_DOWN;
		if (PaddleY[1] > autoy) key1 = HID_KEY_ARROW_UP;
	}

	// left player up
	if ((UsbKeyIsPressed(HID_KEY_Q) && !Comp[0]) || ((Tied || Comp[1]) && UsbKeyIsPressed(HID_KEY_ARROW_UP) && !Comp[0]) || (key0 == HID_KEY_ARROW_UP))
	{
	...
	// left player down
	if ((UsbKeyIsPressed(HID_KEY_A) && !Comp[0]) || ((Tied || Comp[1]) && UsbKeyIsPressed(HID_KEY_ARROW_DOWN) && !Comp[0]) || (key0 == HID_KEY_ARROW_DOWN))
	{
		PaddleY[0] += TimeDelta*PADDLESPEED;
	...
	// right player up
	if ((UsbKeyIsPressed(HID_KEY_ARROW_UP) && !Comp[1]) || ((Tied || Comp[0]) && UsbKeyIsPressed(HID_KEY_Q) && !Comp[1]) || (key1 == HID_KEY_ARROW_UP))
	{
	...
	// right player down
	if ((UsbKeyIsPressed(HID_KEY_ARROW_DOWN) && !Comp[1]) || ((Tied || Comp[0]) && UsbKeyIsPressed(HID_KEY_A) && !Comp[1]) || (key1 == HID_KEY_ARROW_DOWN))
	{
		PaddleY[1] += TimeDelta*PADDLESPEED;
...
#define MENUDY 16
	...
	DrawText("SPACE ..... Play with tied paddles", MENUX, MENUY+5*MENUDY, COL_WHITE);

#define MENUY2 (MENUY+7*MENUDY)
	// draw game control
	DrawText("Game control:", MENUX, MENUY2+0*MENUDY, COL_GREEN);
	DrawText("SPACE ........... Throw the ball", MENUX, MENUY2+1*MENUDY, COL_WHITE);
	DrawText("Q/A ............. Left paddle", MENUX, MENUY2+2*MENUDY, COL_WHITE);
	DrawText("UP/DOWN ......... Right paddle", MENUX, MENUY2+3*MENUDY, COL_WHITE);
	DrawText("ESC ............. Quit the game", MENUX, MENUY2+4*MENUDY, COL_WHITE);
...
void Game(Bool comp1, Bool comp2, Bool tied)
{
	...
	DispAll();
	while (UsbKeyIsPressed(HID_KEY_ARROW_UP) || UsbKeyIsPressed(HID_KEY_ARROW_DOWN) ||
		UsbKeyIsPressed(HID_KEY_A) || UsbKeyIsPressed(HID_KEY_Q) || UsbKeyIsPressed(HID_KEY_TAB) ||
		UsbKeyIsPressed(HID_KEY_SPACE) || UsbKeyIsPressed(HID_KEY_ESCAPE)) {}
	UsbFlushKey();
	...
	while (True)
	{
		...
		// auto serve
		if ((ch == NOCHAR) && ((Time() - LastOut) >= 1000000))
		{
