...
// ADC disable
INLINE void ADC_Disable() { RegClr(ADC_CS, B0); }

// ADC enable
INLINE void ADC_Enable() { RegSet(ADC_CS, B0); }
...
INLINE Bool ADC_IntRaw() { return (*ADC_INTR & B0) != 0; }
...
INLINE void ADC_Force() { RegSet(ADC_INTF, B0); }

// clear forced FIFO interrupt request
INLINE void ADC_ForceClear() { RegClr(ADC_INTF, B0); }

// check if force FIFO interrupt request is set (can be reset with ADC_ForceClear())
INLINE Bool ADC_IsForced() { return (*ADC_INTF & B0) != 0; }

// get interrupt status after masking and forcing
INLINE Bool ADC_IntStatus() { return (*ADC_INTS & B0) != 0; }
