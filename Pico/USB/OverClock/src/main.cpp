...
void ErrLock(const char* msg)
{
	...
	while (True)
	{
		printf("freq %'d Hz vreg=%.2f clk=%d temp=%.2fC  \r",
			ClockGetHz(CLK_PLL_SYS), VregVoltageFloat(), FlashClkDiv(), ADC_Temp());
		WaitMs(500);
	}
}
...
int main()
{
	...
	// delay to connect USB console
	for (i = 5; i > 0; i--)
	{
		printf("Start in %d sec..\r", i);
		WaitMs(1000);
	}
	...
	while (True)
	{
		...
		ClockPllSysFreq(f);
		printf("freq %'d Hz vreg=%.2f clk=%d temp=%.2fC\n",
			ClockGetHz(CLK_PLL_SYS), VregVoltageFloat(), FlashClkDiv(), ADC_Temp());
