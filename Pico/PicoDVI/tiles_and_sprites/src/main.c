...
void NOFLASH(game_init)(game_state_t *state) {
	state->cam_x = 0;
	state->cam_y = 0;
	state->frame_ctr = 0;
...
void NOFLASH(update)(game_state_t *state) {
...
void NOFLASH(render_scanline)(uint16_t *pixbuf, uint y, const game_state_t *gstate) {
	tilebg_t bg = {
...
//uint16_t __scratch_y("render") __attribute__((aligned(4))) 
u16 ALIGNED core0_scanbuf[FRAME_WIDTH];
//uint16_t __scratch_x("render") __attribute__((aligned(4)))
u16 ALIGNED  core1_scanbuf[FRAME_WIDTH];
...
void NOFLASH(encode_scanline)(uint16_t *pixbuf, uint32_t *tmdsbuf) {
