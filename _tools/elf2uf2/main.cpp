...
#define XIP_SRAM_START_RP2350   0x13ffc000
#define XIP_SRAM_END_RP2350     0x14000000
...
const address_ranges rp2350_address_ranges_ram{
    ...
    address_range(0x00000000u, 0x00008000u, address_range::type::IGNORE) // for now we ignore the bootrom if present
};
...
static int usage() {
    fprintf(stderr, "Usage: elf2uf2 (-v) <input ELF file> <output UF2 file> <type 1=RP2040, 2=RP2350ARM, 3=RP2040RISCV>\n");
    return ERROR_ARGS;
}
...
int elf2uf2(FILE *in, FILE *out, char family_id) {
    elf32_header eh;
    ...
    if (!rc) {
        ram_style = is_address_initialized(ram_range, eh.entry);
        if (verbose) {
            if (ram_style) {
...
//    }
    ...
    block.file_size = (family_id == '1') ? RP2040_FAMILY_ID : ((family_id == '2') ? RP2350_ARM_S_FAMILY_ID : RP2350_RISCV_FAMILY_ID);
...
int main(int argc, char **argv) {
    ...
    int rc = elf2uf2(in, out, family_id);
    fclose(in);
    fclose(out);
    if (rc) {
        remove(out_filename);
