...
const sPuzzle PuzzleList[] = {
	{ DigitsImg,			" Digits                   " },
	{ GirlWithKittenImg,		" Girl with Kitten         " },
	{ KittensInBoxImg,		" Kittens in Box           " },
	{ KittensInForestImg,		" Kittens in Forest        " },
	{ KittensOnMeadowImg,		" Kittens on Meadow        " },
	{ KittensWithButterfliesImg,	" Kittens with Butterflies " },
	{ KittensWithFlowersImg,		" Kittens with Flowers     " },
	{ KittensWithSnowdropsImg,	" Kittens with Snowdrops   " },
	{ PegasusImg,			" Pegasus                  " },
	{ PersianKittenImg,		" Persian Kitten           " },
	{ PhoenixImg,			" Phoenix                  " },
	{ SeaDraqonImg,			" Sea Dragon               " },
	{ TwoPersianKittensImg,		" Two Persian Kittens      " },
	{ TwoPuppyDogsImg,		" Two Puppy Dogs           " },
};
...
void DrawBoard()
{
	...
	for (i = 0; i < TILESY; i++)
	{
		for (j = 0; j < TILESX; j++)
		{
			...
			{
				DrawImg(TilesImg + TILE_EMPTY*TILEW*TILEH, x, y, TILEW, TILEH, TILEW);
			}
			else
			{
				// draw result tile
				bx = b & 3;
				by = b >> 2;
				DrawImg(Img + bx*TILEW + by*IMGW*TILEH, x, y, TILEW, TILEH, IMGW);
...
void WaitOn()
{
	...
	DrawImg(TilesImg + TILE_WAIT*TILEW*TILEH, x, y, TILEW, TILEH, TILEW);
	DispUpdate();
}
...
void WaitOn2()
{
	int x = HoleX()*TILEW+BOARDX;
	int y = HoleY()*TILEH+BOARDY;
	DrawImg(TilesImg + TILE_WAIT2*TILEW*TILEH, x, y, TILEW, TILEH, TILEW);
	DispUpdate();
}
