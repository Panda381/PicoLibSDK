...
// save screen content
u8 SaveCardBuf[CARDW*CARDH];
u8 SaveCardBuf2[CARDW*CARDH];
u8 SaveCloudBuf[CLOUDW*CLOUDH*PAR_NUM*2];
u8 SaveWinBuf[WINW*WINH];
...
void SaveCard(u8* buf, int x, int y)
{
	int h;
	u8* s = &BackBuf[x + y*WIDTH];
	for (h = CARDH; h > 0; h--)
	{
		memcpy(buf, s, CARDW);
		buf += CARDW;
			...
			for (par = 0; par < PAR_NUM; par++)
			{
				...
				if (p->add[par] != 0)
				{
					...
					if (p->add[par] >= 0)
					{
						...
						DrawText(NumBuf, x+15-n*4, y+6, COL_BLUE);
					}
					else
					{
						n = DecNum(NumBuf, p->add[par], 0);
						DrawText(NumBuf, x+15-n*4, y+6, COL_RED);
					}
					save += CLOUDW*CLOUDH;
				}
				y += STATEDY;
...
#else
		DrawText("Press BackSpace to quit the game", 32, 220, COL_WHITE);
#endif
