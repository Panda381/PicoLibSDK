...
// CPU
sI4004 i4004cpu;
u8 Memory[I4004_ROMSIZE]; // memory 4 KB
...
// write byte to ROM
void FASTCODE NOFLASH(SetRomMem)(u16 addr, u8 data) { }
...
#define DEBSYN_Y	194
		...
		if (mx < used) mx = used;
		if (mx > 999) mx = 999;
		i = MemPrint(Buf, BUFN, "PC=%04X CPU used=%d%% CPU max=%d%%", i4004cpu.pc, used, mx);
...
void DispBase(u32 freq, u32 sysclk)
{
	...
	// frequency
	i = MemPrint(Buf, BUFN, "f=%d sysclk=%d div=%#f", freq, sysclk, (double)sysclk/(freq*I4004_CLOCKMUL));
...
int main()
{
	...
	FileSelInit(SampPath, "I4004", "BIN", &FileSelColGreen);
	...
	// ---- select and load sample program
		...
		else
		{
			...
			if (s > I4004_ROMSIZE) s = I4004_ROMSIZE;
