...
// CPU
sI8085	i8085cpu;
u8 Memory[I8085_MEMSIZE]; // memory 64 KB
...
// read Serial Input Data Bit (returns 0 or 1)
u8 FASTCODE NOFLASH(EmuReadSid)()
{
	return 0;
}

// write Serial Output Data Bit (val = 0 or 1)
void FASTCODE NOFLASH(EmuWriteSod)(u8 val) {}
...
#define DEBSYN_Y	194
		...
		if (mx < used) mx = used;
		if (mx > 999) mx = 999;
		i = MemPrint(Buf, BUFN, "PC=%04X CPU used=%d%% CPU max=%d%%", i8085cpu.pc, used, mx);
...
void DispBase(u32 freq, u32 sysclk)
{
	...
	// frequency
	i = MemPrint(Buf, BUFN, "f=%d sysclk=%d div=%#f", freq, sysclk, (double)sysclk/(freq*I8085_CLOCKMUL));
...
void EmuDevInit()
{
	...
	// initialize I8085 table
	I8085_InitTab();

	// setup callback functions
	i8085cpu.readmem = EmuGetMem;
	i8085cpu.writemem = EmuSetMem;
	i8085cpu.readport = EmuGetPort;
	i8085cpu.writeport = EmuSetPort;
	i8085cpu.readsid = EmuReadSid;
	i8085cpu.writesod = EmuWriteSod;
...
int main()
{
	...
	u32 sysclk = PWM_FindSysClk(EMU_CLKSYS_MIN*MHZ, EMU_CLKSYS_MAX*MHZ, EMU_FREQ*I8085_CLOCKMUL);
	...
	FileSelInit(SampPath, "I8085", "BIN", &FileSelColGreen);
	...
	// ---- select and load sample program
		...
		else
		{
			...
			if (s > I8085_MEMSIZE) s = I8085_MEMSIZE;
			...
			// run program
			u32 freq = I8085_Start(&i8085cpu, EMU_PWM, EMU_FREQ);
