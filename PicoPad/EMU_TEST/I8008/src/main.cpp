...
#define EMU_FREQ	500000	// emulation frequency
...
// CPU
sI8008	i8008cpu;
u8 Memory[I8008_MEMSIZE]; // memory 16 KB
...
#define DEBSYN_Y	194
		...
		if (mx < used) mx = used;
		if (mx > 999) mx = 999;
		i = MemPrint(Buf, BUFN, "PC=%04X CPU used=%d%% CPU max=%d%%", i8008cpu.pc, used, mx);
...
void DispBase(u32 freq, u32 sysclk)
{
	...
	// frequency
	i = MemPrint(Buf, BUFN, "f=%d sysclk=%d div=%#f", freq, sysclk, (double)sysclk/(freq*I8008_CLOCKMUL));
...
void EmuDevInit()
{
	...
	// initialize I8008 table
	I8008_InitTab();

	// setup callback functions
	i8008cpu.readmem = EmuGetMem;
	i8008cpu.writemem = EmuSetMem;
	i8008cpu.readport = EmuGetPort;
	i8008cpu.writeport = EmuSetPort;
...
int main()
{
	...
	u32 sysclk = PWM_FindSysClk(EMU_CLKSYS_MIN*MHZ, EMU_CLKSYS_MAX*MHZ, EMU_FREQ*I8008_CLOCKMUL);
	...
	FileSelInit(SampPath, "I8008", "BIN", &FileSelColGreen);
	...
	// ---- select and load sample program
		...
		else
		{
			...
			if (s > I8008_MEMSIZE) s = I8008_MEMSIZE;
			...
			// run program
			u32 freq = I8008_Start(&i8008cpu, EMU_PWM, EMU_FREQ);
