...
#define EMU_FREQ	4194304	// emulation frequency
#define EMU_CLKSYS_MIN	120	// minimal system clock in MHz
#define EMU_CLKSYS_MAX	180	// maximal system clock in MHz

// Emulator ports (memory mapped at X80_PORTBASE port base, to range 0xFF80..0xFF8F):
//
//   LDH (8fh),a: output to hardware USR LED on PicoPad panel
//		bit 0 ... set LED output (1=ON, 0=OFF), only if bit 1 is 0
//		bit 1 ... 1=flip LED output, in this case bit 0 is ignored
//
//   LDH (8eh),a: output to LEDs, bit 0 to 7 = LED 0 to 7, 1=LED is ON
//
//   LDH (8dh),a: write digit to output shift register of the 7-segment display (10-digit positions)
//		values 0 to DIG_MAX: digit 0 to 9, A to Z, space, segments, ...
//
//   LDH (8ch),a: any write - copy digits from output shift register to 7-segment display
//
//   LDH (8bh),a: position of decimal point 0..9, set other value to hide decimal point
//
//   LDH (8ah),a: write ASCII character to output text buffer (20 characters of scrolling title from right to left)
//
//   LDH (89h),a: output to speaker
//		bit 0 ... output to speaker (if bit 1 is 0)
//		bit 1 ... flip output to speaker (bit 0 is ignored)
//
//   LDH a,(87h): input keys, 1=key is pressed
//		bit 0 ... DOWN
//		bit 1 ... RIGHT
//		bit 2 ... LEFT
//		bit 3 ... UP
...
// CPU
sX80	x80cpu;
u8 Memory[X80_MEMSIZE]; // memory 64 KB
...
{
	// read memory
	if (addr < X80_PORTBASE) return Memory[addr];

	// get keys
	if (addr == (X80_PORTBASE + 0x87)) return Keys;
...
void FASTCODE NOFLASH(EmuSetMem)(u16 addr, u8 data)
{
	...
	else
	{
		...
		// output to LEDs
		else if (addr == (X80_PORTBASE + 0x8e))
		{
			LEDs = data;
		}

		// output digit to the shift register
		else if (addr == (X80_PORTBASE + 0x8d))
		{
			u32 k, k2;
		...
		else if (addr == (X80_PORTBASE + 0x8c))
		{
		...
		else if (addr == (X80_PORTBASE + 0x8b))
		{
			DigDP = data;
		}

		// output ASCII character to the title
		else if (addr == (X80_PORTBASE + 0x8a))
		{
			u32 k, k2;
			u32* d = (u32*)TitleBuf;
			k = d[4];
		...
		else if (addr == (X80_PORTBASE + 0x89))
		{
#ifdef PWMSND_GPIO
			if ((data & B1) != 0)
...
#define DEBSYN_Y	194
		...
		if (mx < used) mx = used;
		if (mx > 999) mx = 999;
		i = MemPrint(Buf, BUFN, "PC=%04X CPU used=%d%% CPU max=%d%%", x80cpu.pc, used, mx);
...
void DispBase(u32 freq, u32 sysclk)
{
	...
	// frequency
	i = MemPrint(Buf, BUFN, "f=%d sysclk=%d div=%#f", freq, sysclk, (double)sysclk/(freq*X80_CLOCKMUL));
...
int main()
{
	...
	u32 sysclk = PWM_FindSysClk(EMU_CLKSYS_MIN*MHZ, EMU_CLKSYS_MAX*MHZ, EMU_FREQ*X80_CLOCKMUL);
	...
	FileSelInit(SampPath, "X80", "BIN", &FileSelColGreen);
	...
	// ---- select and load sample program
		...
		else
		{
			...
			if (s > X80_MEMSIZE) s = X80_MEMSIZE;
			...
			// run program
			u32 freq = X80_Start(&x80cpu, EMU_PWM, EMU_FREQ);
