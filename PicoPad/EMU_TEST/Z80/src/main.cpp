...
#define EMU_FREQ	4000000	// emulation frequency
...
#define DEBSYN_Y	194
		...
		if (mx < used) mx = used;
		if (mx > 999) mx = 999;
		i = MemPrint(Buf, BUFN, "PC=%04X CPU used=%d%% CPU max=%d%%", z80cpu.pc, used, mx);
...
void DispBase(u32 freq, u32 sysclk)
{
	...
	// frequency
	i = MemPrint(Buf, BUFN, "f=%d sysclk=%d div=%#f", freq, sysclk, (double)sysclk/(freq*Z80_CLOCKMUL));
...
int main()
{
	...
	u32 sysclk = PWM_FindSysClk(EMU_CLKSYS_MIN*MHZ, EMU_CLKSYS_MAX*MHZ, EMU_FREQ*Z80_CLOCKMUL);
	...
	FileSelInit(SampPath, "Z80", "BIN", &FileSelColGreen);
	...
	// ---- select and load sample program
		...
		else
		{
			...
			if (s > Z80_MEMSIZE) s = Z80_MEMSIZE;
			...
			// run program
			u32 freq = Z80_Start(&z80cpu, EMU_PWM, EMU_FREQ);
