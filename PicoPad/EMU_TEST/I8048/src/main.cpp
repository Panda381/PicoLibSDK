...
//		bit 7 ... Y
//
//   OUTL BUS,A: write ASCII character to output text buffer (20 characters of scrolling title from right to left)
//
//   OUTL P1,A: output to LEDs, bit 0 to 7 = LED 0 to 7, 1=LED is ON
//
//   OUTL P2,A:
...
// CPU
sI8048	i8048cpu;
u8 Memory[I8048_ROMSIZE]; // ROM memory 4 KB
...
u8 FASTCODE NOFLASH(EmuGetExt)(u8 addr) { return 0xff; }

// write to extended data memory
void FASTCODE NOFLASH(EmuSetExt)(u8 addr, u8 data) { }
...
// output PROG signal
void FASTCODE NOFLASH(EmuSetProg)(u8 data) { }

// read test signal
u8 FASTCODE NOFLASH(EmuGetTest)(u8 addr) { return 1; }
...
#define DEBSYN_Y	194
		...
		if (mx < used) mx = used;
		if (mx > 999) mx = 999;
		i = MemPrint(Buf, BUFN, "PC=%04X CPU used=%d%% CPU max=%d%%", i8048cpu.pc, used, mx);
...
void DispBase(u32 freq, u32 sysclk)
{
	...
	// frequency
	i = MemPrint(Buf, BUFN, "f=%d sysclk=%d div=%#f", freq, sysclk, (double)sysclk/(freq*I8048_CLOCKMUL));
...
void EmuDevInit()
{
	...
	i8048cpu.readrom = EmuGetRom;
	i8048cpu.readext = EmuGetExt;
	i8048cpu.writeext = EmuSetExt;
	i8048cpu.readport = EmuGetPort;
	i8048cpu.writeport = EmuSetPort;
	i8048cpu.writeprog = EmuSetProg;
	i8048cpu.readtest = EmuGetTest;
...
int main()
{
	...
	u32 sysclk = PWM_FindSysClk(EMU_CLKSYS_MIN*MHZ, EMU_CLKSYS_MAX*MHZ, EMU_FREQ*I8048_CLOCKMUL);
	...
	FileSelInit(SampPath, "I8048", "BIN", &FileSelColGreen);
	...
	// ---- select and load sample program
		...
		else
		{
			...
			if (s > I8048_ROMSIZE) s = I8048_ROMSIZE;
			...
			// run program
			u32 freq = I8048_Start(&i8048cpu, EMU_PWM, EMU_FREQ);
