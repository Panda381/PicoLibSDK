...
struct Map9_Latch 	// size 4 bytes
{
	BYTE lo_bank;
	BYTE hi_bank;
	BYTE state;
...
typedef struct {
	struct Map9_Latch	md_latch1;	// 0x00
	struct Map9_Latch	md_latch2;	// 0x04
} sMap9_Data;

STATIC_ASSERT((sizeof(sMap9_Data) == 0x08) && (sizeof(sMap9_Data) <= MAPDATA_SIZE), "Incorrect sMap9_Data!");

#define MAPDATA9	((sMap9_Data*)NES->mapdata)	// mapper data

#define latch1		MAPDATA9->md_latch1
#define latch2		MAPDATA9->md_latch2
