...
for (uint8_t B_=0;B_<8;B_++){
            if (Byte_& (1 << B_)) {
				uint16_t Pix_ = Col_;
            uint16_t x2 = (XPASS * 2) + Off_setX;
            uint16_t y2 = (YPassMul * 2) + Off_setY+(B_*2);

            // Utilisé un seul accès à FrameBuf pour mettre à jour les pixels
      /*
             DrawPoint(x2,y2,Pix_);		
             DrawPoint(x2,y2+1,Pix_);		
             DrawPoint(x2+1,y2,Pix_);		
             DrawPoint(x2+1,y2+1,Pix_);		
       */
...
uint16_t MainBufferColor(uint8_t add2_){
	switch(Back_Color){
		case 0:return  color(31,31+(add2_),12); break;
		case 1:return  color(add2_,31+add2_,31); break;
		case 2:return  color(31-add2_,31+(add2_),31); break;
		default:return color(31,63,31);break;
}}

void PICOCOMPATIBILITY::DrawBuffer(void) {
    #define Off_setX 32
    #define Off_setY 48
    // Dimension des buffers
    const uint16_t WIDTH_MOD = WIDTH; // Eviter la répétition
    ...
    for (y = 0; y < 64; y++) {
        for (x = 0; x < 128; x++) {
			uint16_t Pix_ = (ExploreMap(x, y)) ? MainBufferColor(add_) : 0;
...
void PICOCOMPATIBILITY::transferBufferToColorDisplay_320x240() {
    ...
    for (int yOrig = 0; yOrig < 64; ++yOrig) {
        for (int xOrig = 0; xOrig < 128; ++xOrig) {
            ...
            int yScreen = 48 + yOrig;
            int xScreen = 32 + xOrig;
