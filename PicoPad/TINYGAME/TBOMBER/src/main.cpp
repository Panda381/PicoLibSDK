...
if (BUTTON_DOWN) {Sound(100,255);Sound(40,255);_delay_ms(600);StartGame();break;}
}
NEWLEVEL:
if (Level<2) {
if (Level>-1) {
  {for(uint8_t t=0;t<=4;t++){Sound(80,100);delay(300);}}
  }
Level++;
}else{
{for(uint8_t t=0;t<=4;t++){Sound(80,100);delay(300);}}
SPEEDBOMBER=1;Level=0;}
resetMonster(Sprite);
LoadLevel(Level);
RESTARTLEVEL:
resetBomb();
...
else if (TINYJOYPAD_RIGHT) {Sprite[0].DirectionV=1;}else{if (((Sprite[0].x)%8)==0) Sprite[0].DirectionV=2;}
if (TINYJOYPAD_DOWN) {Sprite[0].DirectionH=1;}
else if (TINYJOYPAD_UP) {Sprite[0].DirectionH=0;}else{if (Sprite[0].Decalagey==0) Sprite[0].DirectionH=2;}
...
for (y = 0; y < 8; y++){ 
for (x = 0; x < 128; x++){
		TinyOLED_Send(x,y,(PrintLive(x,y)&(background(x,y))));
}}  
...
if (INGAME) {
	...
	DirectDraw(BombBlitz(x,y),x,y,0);
	DirectDraw(Explose(x,y)|SpriteWrite(x,y,Sprite),x,y,color(31,32,15));
	DirectDraw(BlockBomb(x,y,Div2x),x,y,color(25,51,25));
	DirectDraw(SpriteMain(x,y,Sprite),x,y,color(20,52,31));
	}
	}else if (render0_picture1==1){
TinyOLED_Send(x,y,(BackBlitz[x+(y*128)]));}Div2x=!Div2x;
}
}
...
return caracters[((x-Sprite[SpriteNumber].x)+(8*(Sprite[SpriteNumber].type*12)))+(Sprite[SpriteNumber].anim*8)+(Sprite[SpriteNumber].DirectionAnim*8)];
}return 0;}
