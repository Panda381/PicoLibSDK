...
#define DEBOUNCE(ZELTADA)  _delay_ms(30);if (ZELTADA) // exemple:" if (BUTTON_DOWN) {DEBOUNCE(BUTTON_DOWN
...
uint8_t blitzSprite(int8_t xPos, int8_t yPos, uint8_t xPASS, uint8_t yPASS, uint8_t FRAME, const uint8_t *SPRITES) {
  ...
  if (ScanA > Wmax) {
    ...
    OUTBYTE = SplitSpriteDecalageY(SPRITEyDECALAGE, SPRITES[ScanA + (PICBYTE)], 1);
  }
  if (SPRITEyLINE > 0) {
...
void FADE(const uint8_t *PIC, uint8_t FADE) {
  ...
  for (l = 0; l < 9; l++) {
    ...
    for (y = 0; y < 8; y++) {
      ...
      for (x = 0; x < 128; x++) {
        PICOKIT.TinyOLED_Send(x,y,l2 & PIC[x + ((y)*128)]);
      }
